...
namespace CSharpExercises
{
    public static class Exercises
    {
        ...
        {
            string triangle = "";
            int widthHorizontal = width;
            for (int i = 0; i < width; i++)
            {
                for (int j = widthHorizontal; j > 0; j--)
                {
        ...
        public static string GetMilesPerHour (double dist, int hours, int mins, int sec)
        {
            int timeInSec = 0;
            double timeInHour = 0;
            double milesPerHour;
            ...
            timeInSec = sec + (mins * 60) + (hours * 3600);
            timeInHour = timeInSec / 3600.0;
            milesPerHour = dist / timeInHour;
            milesPerHour = Math.Round(milesPerHour, 0);
            speed = milesPerHour.ToString() + "MPH";
            return speed;
        ...
        {
            string[] wordArg = { word };
            string[] words = text.Split(wordArg, StringSplitOptions.None);
        ...
        public static decimal MortgageCalculator (decimal loanBalance, decimal interestRate, int loanTerm, int period)
        {
            int numOfPayments = loanTerm * period;
            decimal monthlyInterestRate = (interestRate / 100) / period;
            decimal compoundedInterestRate = (decimal)Math.Pow((double)(1 + monthlyInterestRate), numOfPayments);
         ...
         * Goose
         */
         public static string DuckGoose (int end)
        {
            int i = 1;
            string output = "";
            while( i <= end )
